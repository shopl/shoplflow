...
function buildSetup() {
  ...
  const providerSrc = read(path.join(BASE, 'src/providers/ShoplflowProvider.tsx'));
  ...
  const providerName = (providerSrc.match(/const\s+(\w+)\s*=/) || [])[1] || 'ShoplflowProvider';
  const domainDefault = (providerSrc.match(/domain\s*=\s*['"]([A-Za-z]+)['"]/) || [])[1] || null;
  const domainValues = [...domainSrc.matchAll(/'([A-Za-z]+)'/g)].map((m) => m[1]);
  ...
  for (const [key, target] of Object.entries(pkg.exports || {})) {
    ...
    styles.push({
      ...
      role: key === './reset' ? 'CSS reset (optional)' : 'design-token CSS vars + typography classes (required)',
    });
  }

  // Why the provider/components are client-only (derived from the provider's own imports).
  const clientReasons = [];
  if (/framer-motion/.test(providerSrc)) clientReasons.push('framer-motion (LazyMotion)');
  if (/Portal/.test(providerSrc)) clientReasons.push('Modal/Popper portals');
  if (/useDomain/.test(providerSrc)) clientReasons.push('DOM theming hook (data-shoplflow)');
  ...
  const provider = {
    ...
    clientOnly: true,
    clientReasons,
    props: [{ name: 'domain', type: 'DomainType', optional: true, default: domainDefault, values: domainValues }],
  };
  ...
  return {
    source: ['packages/base/package.json', 'ShoplflowProvider.tsx', 'Domain.ts', 'setup.curated.json'],
    provider,
    styles,
