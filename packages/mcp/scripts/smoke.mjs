...
const prim = await client.callTool({
  name: "get_token",
  arguments: { name: "primary300" },
});
console.log(
  "\nget_token(primary300):",
  prim.content[0].text.replace(/\s+/g, " ").slice(0, 200),
);

const miss = await client.callTool({
  name: "get_token",
  arguments: { name: "primaryX" },
});
console.log(
  "\nget_token(primaryX) [miss w/ suggestions]:",
  miss.content[0].text.replace(/\s+/g, " "),
);

const iconHit = await client.callTool({
  name: "search_icon",
  arguments: { query: "chat bot" },
});
...
const iconBrand = await client.callTool({
  name: "search_icon",
  arguments: { query: "add", domain: "hada", limit: 3 },
});
console.log(
  '\nsearch_icon("add", hada, 3):',
  iconBrand.content[0].text.replace(/\s+/g, " "),
);

const compSearch = await client.callTool({
...
const compApi = await client.callTool({
...
console.log(
  "  first:",
  JSON.stringify(usageParsed.examples[0]).replace(/\s+/g, " ").slice(0, 140),
);

const usageCompound = await client.callTool({
  name: "get_usage_example",
  arguments: { name: "ModalContainer" },
});
...
const env = await client.callTool({
  ...
  arguments: { framework: "next app router" },
});
