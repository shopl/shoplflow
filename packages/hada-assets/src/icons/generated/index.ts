import English from './English';
import Korean from './Korean';
import IcAdd from './IcAdd';
import IcAdmin from './IcAdmin';
import IcAlbum from './IcAlbum';
import IcAlert from './IcAlert';
import IcCalendar from './IcCalendar';
import IcCamera from './IcCamera';
import IcCancel from './IcCancel';
import IcCheckActive from './IcCheckActive';
import IcCheckDefault from './IcCheckDefault';
import IcCheck from './IcCheck';
import IcCheckbox from './IcCheckbox';
import IcChecklist from './IcChecklist';
import IcClose from './IcClose';
import IcCompleted from './IcCompleted';
import IcCopy from './IcCopy';
import IcCrew from './IcCrew';
import IcDelete from './IcDelete';
import IcDownArrowSolid from './IcDownArrowSolid';
import IcDownArrow from './IcDownArrow';
import IcDownload from './IcDownload';
import IcDrag from './IcDrag';
import IcDrawer from './IcDrawer';
import IcEdit from './IcEdit';
import IcEmail from './IcEmail';
import IcError from './IcError';
import IcExcel from './IcExcel';
import IcFilter from './IcFilter';
import IcFinger from './IcFinger';
import IcGradeBadge from './IcGradeBadge';
import IcHelpCenter from './IcHelpCenter';
import IcHelp from './IcHelp';
import IcHome from './IcHome';
import IcInfo from './IcInfo';
import IcIssue from './IcIssue';
import IcIssues from './IcIssues';
import IcLanguage from './IcLanguage';
import IcLeftArrow from './IcLeftArrow';
import IcLink from './IcLink';
import IcListEdit from './IcListEdit';
import IcLogout from './IcLogout';
import IcMemo from './IcMemo';
import IcMenu from './IcMenu';
import IcMobileview from './IcMobileview';
import IcMore from './IcMore';
import IcNext from './IcNext';
import IcNoticeLine from './IcNoticeLine';
import IcNotice from './IcNotice';
import IcOverview from './IcOverview';
import IcPassword from './IcPassword';
import IcPhone from './IcPhone';
import IcPlace from './IcPlace';
import IcQrcode from './IcQrcode';
import IcRadio from './IcRadio';
import IcRefresh from './IcRefresh';
import IcRemove from './IcRemove';
import IcRightArrow from './IcRightArrow';
import IcSearch from './IcSearch';
import IcSection from './IcSection';
import IcSettingFill from './IcSettingFill';
import IcSetting from './IcSetting';
import IcSheet from './IcSheet';
import IcSlider from './IcSlider';
import IcStatisticActive from './IcStatisticActive';
import IcStatisticDefault from './IcStatisticDefault';
import IcStatistics from './IcStatistics';
import IcText from './IcText';
import IcTime from './IcTime';
import IcTutorialCompleted from './IcTutorialCompleted';
import IcTutorial from './IcTutorial';
import IcUpArrowSolid from './IcUpArrowSolid';
import IcUpArrow from './IcUpArrow';
import IcUser from './IcUser';
import IcViewOff from './IcViewOff';
import IcViewOn from './IcViewOn';
import IcWorker from './IcWorker';
import IcYesno from './IcYesno';
import ImgAlert from './ImgAlert';
import ImgAssignees from './ImgAssignees';
import ImgAverage from './ImgAverage';
import ImgCheckedFacility from './ImgCheckedFacility';
import ImgChecklist from './ImgChecklist';
import ImgFacility from './ImgFacility';
import ImgPending from './ImgPending';
import ImgResolved from './ImgResolved';
import ImgScheduled from './ImgScheduled';
import ImgTutorial01 from './ImgTutorial01';
import ImgTutorial02 from './ImgTutorial02';
import ImgTutorial03 from './ImgTutorial03';
import ImgUpgrade from './ImgUpgrade';

const icons = {
  english: English,
  korean: Korean,
  add: IcAdd,
  admin: IcAdmin,
  album: IcAlbum,
  alert: IcAlert,
  calendar: IcCalendar,
  camera: IcCamera,
  cancel: IcCancel,
  'check-active': IcCheckActive,
  'check-default': IcCheckDefault,
  check: IcCheck,
  checkbox: IcCheckbox,
  checklist: IcChecklist,
  close: IcClose,
  completed: IcCompleted,
  copy: IcCopy,
  crew: IcCrew,
  delete: IcDelete,
  'down-arrow-solid': IcDownArrowSolid,
  'down-arrow': IcDownArrow,
  download: IcDownload,
  drag: IcDrag,
  drawer: IcDrawer,
  edit: IcEdit,
  email: IcEmail,
  error: IcError,
  excel: IcExcel,
  filter: IcFilter,
  finger: IcFinger,
  'grade-badge': IcGradeBadge,
  'help-center': IcHelpCenter,
  help: IcHelp,
  home: IcHome,
  info: IcInfo,
  issue: IcIssue,
  issues: IcIssues,
  language: IcLanguage,
  'left-arrow': IcLeftArrow,
  link: IcLink,
  'list-edit': IcListEdit,
  logout: IcLogout,
  memo: IcMemo,
  menu: IcMenu,
  mobileview: IcMobileview,
  more: IcMore,
  next: IcNext,
  'notice-line': IcNoticeLine,
  notice: IcNotice,
  overview: IcOverview,
  password: IcPassword,
  phone: IcPhone,
  place: IcPlace,
  qrcode: IcQrcode,
  radio: IcRadio,
  refresh: IcRefresh,
  remove: IcRemove,
  'right-arrow': IcRightArrow,
  search: IcSearch,
  section: IcSection,
  'setting-fill': IcSettingFill,
  setting: IcSetting,
  sheet: IcSheet,
  slider: IcSlider,
  'statistic-active': IcStatisticActive,
  'statistic-default': IcStatisticDefault,
  statistics: IcStatistics,
  text: IcText,
  time: IcTime,
  'tutorial-completed': IcTutorialCompleted,
  tutorial: IcTutorial,
  'up-arrow-solid': IcUpArrowSolid,
  'up-arrow': IcUpArrow,
  user: IcUser,
  'view-off': IcViewOff,
  'view-on': IcViewOn,
  worker: IcWorker,
  yesno: IcYesno,
  'img-alert': ImgAlert,
  'img-assignees': ImgAssignees,
  'img-average': ImgAverage,
  'img-checked-facility': ImgCheckedFacility,
  'img-checklist': ImgChecklist,
  'img-facility': ImgFacility,
  'img-pending': ImgPending,
  'img-resolved': ImgResolved,
  'img-scheduled': ImgScheduled,
  'img-tutorial-01': ImgTutorial01,
  'img-tutorial-02': ImgTutorial02,
  'img-tutorial-03': ImgTutorial03,
  'img-upgrade': ImgUpgrade,
};

export type IconNames = keyof typeof icons;

export {
  English as EnglishIcon,
  Korean as KoreanIcon,
  IcAdd as AddIcon,
  IcAdmin as AdminIcon,
  IcAlbum as AlbumIcon,
  IcAlert as AlertIcon,
  IcCalendar as CalendarIcon,
  IcCamera as CameraIcon,
  IcCancel as CancelIcon,
  IcCheckActive as CheckActiveIcon,
  IcCheckDefault as CheckDefaultIcon,
  IcCheck as CheckIcon,
  IcCheckbox as CheckboxIcon,
  IcChecklist as ChecklistIcon,
  IcClose as CloseIcon,
  IcCompleted as CompletedIcon,
  IcCopy as CopyIcon,
  IcCrew as CrewIcon,
  IcDelete as DeleteIcon,
  IcDownArrowSolid as DownArrowSolidIcon,
  IcDownArrow as DownArrowIcon,
  IcDownload as DownloadIcon,
  IcDrag as DragIcon,
  IcDrawer as DrawerIcon,
  IcEdit as EditIcon,
  IcEmail as EmailIcon,
  IcError as ErrorIcon,
  IcExcel as ExcelIcon,
  IcFilter as FilterIcon,
  IcFinger as FingerIcon,
  IcGradeBadge as GradeBadgeIcon,
  IcHelpCenter as HelpCenterIcon,
  IcHelp as HelpIcon,
  IcHome as HomeIcon,
  IcInfo as InfoIcon,
  IcIssue as IssueIcon,
  IcIssues as IssuesIcon,
  IcLanguage as LanguageIcon,
  IcLeftArrow as LeftArrowIcon,
  IcLink as LinkIcon,
  IcListEdit as ListEditIcon,
  IcLogout as LogoutIcon,
  IcMemo as MemoIcon,
  IcMenu as MenuIcon,
  IcMobileview as MobileviewIcon,
  IcMore as MoreIcon,
  IcNext as NextIcon,
  IcNoticeLine as NoticeLineIcon,
  IcNotice as NoticeIcon,
  IcOverview as OverviewIcon,
  IcPassword as PasswordIcon,
  IcPhone as PhoneIcon,
  IcPlace as PlaceIcon,
  IcQrcode as QrcodeIcon,
  IcRadio as RadioIcon,
  IcRefresh as RefreshIcon,
  IcRemove as RemoveIcon,
  IcRightArrow as RightArrowIcon,
  IcSearch as SearchIcon,
  IcSection as SectionIcon,
  IcSettingFill as SettingFillIcon,
  IcSetting as SettingIcon,
  IcSheet as SheetIcon,
  IcSlider as SliderIcon,
  IcStatisticActive as StatisticActiveIcon,
  IcStatisticDefault as StatisticDefaultIcon,
  IcStatistics as StatisticsIcon,
  IcText as TextIcon,
  IcTime as TimeIcon,
  IcTutorialCompleted as TutorialCompletedIcon,
  IcTutorial as TutorialIcon,
  IcUpArrowSolid as UpArrowSolidIcon,
  IcUpArrow as UpArrowIcon,
  IcUser as UserIcon,
  IcViewOff as ViewOffIcon,
  IcViewOn as ViewOnIcon,
  IcWorker as WorkerIcon,
  IcYesno as YesnoIcon,
  ImgAlert as ImgAlertIcon,
  ImgAssignees as ImgAssigneesIcon,
  ImgAverage as ImgAverageIcon,
  ImgCheckedFacility as ImgCheckedFacilityIcon,
  ImgChecklist as ImgChecklistIcon,
  ImgFacility as ImgFacilityIcon,
  ImgPending as ImgPendingIcon,
  ImgResolved as ImgResolvedIcon,
  ImgScheduled as ImgScheduledIcon,
  ImgTutorial01 as ImgTutorial01Icon,
  ImgTutorial02 as ImgTutorial02Icon,
  ImgTutorial03 as ImgTutorial03Icon,
  ImgUpgrade as ImgUpgradeIcon,
};

export default icons;
