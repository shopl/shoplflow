...
function processTypographyTokens(tokens, _domain) {
  ...
  Object.keys(tokens).forEach((key) => {
    ...
    if (value.type === "typography") {
      ...
      const mappingFontWeight = fontWeight
        .replace(/{|}/g, "")
        .split(".")
    ...
    const primaryKeysObject = Object.keys(hadaOrShoplTokens)
      ...
      .reduce((acc, cur) => {
        ...
        if (nowToken.value.startsWith("{")) {
          const [first, second] = nowToken.value
            .replace(/^\{|\}$/g, "")
            .split(".");
...
function processBoxShadowTokens(tokens) {
  Object.keys(tokens).forEach((key) => {
    ...
    if (value.type === "boxShadow") {
      console.log(value);
      boxShadowTokens[value.value.type] =
        `${value.value.x}px ${value.value.y}px ${value.value.blur}px ${value.value.spread}px ${value.value.color}`;
    }
  });
...
function separateTokens(tokens) {
  ...
  if (tokens.shopl) {
    shoplTokens.typographyTokens = processTypographyTokens(
      tokens.shopl,
      "shopl",
    );
  }
