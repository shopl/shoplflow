...
const bumpCategories = new Set(
  [...codeCategories].filter((c) => !manualCategories.has(c)),
);
...
if (manualCategories.size > 0) {
  ...
  execSync("git add packages/base/VERSIONING.md", {
    cwd: ROOT,
    stdio: "inherit",
  });
}
...
if (bumpCategories.size > 0) {
  ...
  execSync(`node scripts/component-version.mjs bump ${list}`, {
    cwd: BASE_PKG,
    stdio: "inherit",
  });
  execSync("git add packages/base/VERSIONING.md", {
  ...
  for (const cat of bumpCategories) {
    execSync(`git add packages/base/src/components/${cat}/version.ts`, {
      cwd: ROOT,
      stdio: "inherit",
    });
  }
