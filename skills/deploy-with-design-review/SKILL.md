...
| 5 | 시스템 자동화 | Cloudflare Worker → GitHub `workflow_dispatch` → **같은 파일** `changesets-workflow.yml`(먼저 머지 잡) → `main` push 시 같은 워크플로의 Release 잡이 npm 배포 |
...
2. **Worker → GitHub API**: `workflow_dispatch`로 `changesets-workflow.yml` 실행 (`inputs.branch` = 기능 브랜치)
3. **GitHub Actions** ([.github/workflows/changesets-workflow.yml](../../../.github/workflows/changesets-workflow.yml)): `workflow_dispatch`로 머지 잡 실행 → feature를 main에 머지
4. **같은 워크플로** `push(main)` 트리거: Release 잡 → `version → npm publish`
...
- **main 직접 머지**: PR 없이 머지되므로 `build-test.yml` 등 PR 연동 CI가 실행되지 않을 수 있다. `changesets-workflow`의 디자인 승인 머지 잡에 `pnpm build` 검증 스텝이 포함되어 있다.
